...
class CustomTkinter(customtkinter.CTk):
    ...
    def loop(self):
        global loop
        loop = not loop
        if loop == False: print("Looping OFF")
        if loop == True: print("Looping ON")
    def open(self):
        global files
        files.clear()
    ...
    def downloadFLACs(self):
        ...
        for FLAC in toDownload:
            ...
            try:
                ...
                print(
                    f"\x1b[31mffmpeg is not installed, please install here: https://ffmpeg.org/"
                )
            except Exception as e:
